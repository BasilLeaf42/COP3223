...
void initAuction(float auction[2][MAXAUCTIONITEMS])
{
    ...
    //loop to initialize all variables to 0
...
{
    //variable declaration
    int i, initial_tickets = * ticketsSold;
	
    //loop to determine revenue
    for (i = 0; i < numTickets; i++)
    {
        * totalRevenue += price; 
        
        //loop to determine number of tickets sold
        if (i > 0)
        {
...
{
    //variable declaration
    int i, ticket_count = 0, initial_tickets = 0; 
	
    //loop for new ticket amount if raffle isn't taken
    ...
    //loop to add & assign tickets, and calculate revenue
    for (i = initial_tickets; i < availTickets; i++)
    { 
        while (i < (numTickets + initial_tickets))
        { 
    ...
    //loop to give tickets
    if (initial_tickets <= availTickets)
    {  
        printf("RAFFLE TICKETS %d - %d given to PERSON %d\n", initial_tickets, (ticket_count + initial_tickets - 1), person);
    }

    //if person should not recieve tickets
    else
    {
...
void bid(float auction[2][MAXAUCTIONITEMS], float bid, int auctionNumber, int person, int flag)
{
    //loop to determine if auction is open
    while (flag == 1)
    {
        //if bid is highest
...
float awardAuction(float auction[2][MAXAUCTIONITEMS], int auctionNumber)
{
    //if the auction has more than 0 bids
    if (auction[1][auctionNumber] > 0)
    { 
